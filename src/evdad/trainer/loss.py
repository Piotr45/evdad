...
def get_loss_function(cfg: dict, device: str) -> torch.nn.Module:
    ...
    if loss_function == "SpikeTime":
        ...
        return slayer.loss.SpikeTime(
            time_constant=time_constant, length=length, filter_order=filter_order, reduction=reduction
        ).to(device)
        ...
        return slayer.loss.SpikeRate(
            true_rate=true_rate, false_rate=false_rate, moving_window=moving_window, reduction=reduction
        ).to(device)
    else:
